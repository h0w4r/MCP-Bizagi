...
public sealed class ModelTools(WorkspaceFiles files, ServerOptions options, NativeWorkflows native, Operations operations)
{
    ...
    public CallToolResult Capabilities()
    {
        ...
        return Result(new
        {
            ...
            nativeLiveSession = new { tools = new[] { "live_open", "live_sessions_list", "live_read", "live_apply", "live_history", "live_checkpoint", "live_reconcile", "live_publish", "live_close" },
                requires = "interactive_windows_user_and_on_demand_task_scheduler_activation", automaticLaunch = true,
                ...
                closeScope = "clean_retained_checkpoint_pinned_editor_and_owner_exit_owned_job_cleanup_and_durable_reconciliation",
                scope = "experimental_managed_native_properties_history_checkpoints_publication_and_recovery_not_arbitrary_existing_editor_attachment" },
            ...
            nativeReparenting = new { tool = "native_elements_reparent", maximumBatchSize = 1000,
                crossDiagram = new { requires = new[] { "ExpectedDiagramId", "TargetDiagramId" }, migrates = new[] { "native_subtree", "extended_values", "embedded_files", "images", "current_user_subprocess_tabs" },
                    configuredSimulationMigration = true, presentationActionMigration = true },
            ...
            nativePresentation = new { tools = new[] { "native_presentation_get", "native_presentation_apply" },
                ...
                scope = "experimental_native_action_definitions_and_owned_payloads_with_fresh_readback_not_playback_or_live_documents" },
            nativeAlignment = new { tool = "native_elements_align", modes = NativeAlignmentPolicy.Modes, maximumSelection = 1000,
                attachedBoundaryPolicy = "translate_existing_attached_events_with_moving_hosts_preserving_offsets_labels_and_semantics", directBoundarySelection = false,
            ...
            nativeSurfaceLayout = new { tool = "native_surface_layout", dependency = NativeSurfaceLayoutPlanner.Dependency,
                directions = NativeSurfaceLayoutPlanner.Directions, maximumNodes = 1000, maximumSurfaceRecords = 1000,
                scope = "experimental_server_calculated_closed_unpartitioned_surface_placement_with_installed_native_grouped_moves_routes_and_archive_gate_not_global_or_visual_accreditation" },
            nativeDiagramLayout = new { tool = "native_diagram_layout", dependency = NativeSurfaceLayoutPlanner.Dependency,
                directions = NativeSurfaceLayoutPlanner.Directions, maximumRecords = NativeDiagramLayoutPlanner.MaximumRecords,
            ...
            capabilities = new[] {
                ...
                new { name = "native_explicit_cross_diagram_reparenting", status = "experimental_explicit_subtree_content_tabs_and_mapped_scenario_parameters_three_process_readback_and_whole_archive_gate_with_explicit_presentation_migration_not_layout_or_live_sessions", backend = "bizagi_worker" },
    ...
    [McpServerTool(Name = "native_elements_align"), Description("Experimental native selected-shape alignment or distribution using the installed editor's programmatic engine, not simulated input. Supply native diagram, optional embedded subprocess, 2-1000 distinct element IDs and one of Top, Bottom, Left, Right, Horizontal, Vertical, HorizontalEvenly, VerticalEvenly. Distribution needs at least three nodes. Existing attached boundary events follow moving hosts with preserved offsets and manual labels; do not select boundary events directly. Native callback points and ports are verified independently. Revision checked; original retained. Three workers and whole-archive fidelity gate the output. Cross-owner/semantic changes and unrepresentable results are rejected. Poll operation_get; availability is not universal operational accreditation.")]
    ...
    [McpServerTool(Name = "native_surface_layout"), Description("Experimental server-side automatic placement of every direct node on one closed, unpartitioned Process or embedded SubProcess. Supply DiagramId, OwnerId and Direction (Right or Down); do not supply node coordinates. MSAGL calculates topology positions and host/attached-event/manual-label envelopes. The installed editor applies one grouped move transaction and native routes. Rejects partitions, expanded nodes, cross-owner connections and unsupported content rather than partial layout. Preserves node sizes, host offsets, semantics and unknown archive content with a fresh-worker readback. Revision checked; original retained. Does not promise global routing quality, whole-model layout or desktop visual compatibility. Poll operation_get.")]
    public CallToolResult LayoutNativeSurface(string path, string expectedRevision, NativeSurfaceLayoutRequest layout) => Guard(() => native.LayoutSurface(path, expectedRevision, layout));

    [McpServerTool(Name = "native_diagram_layout"), Description("Experimental server-calculated complete selected-diagram layout. Supply DiagramId and Direction Right or Down, not coordinates. Preserves source lane/milestone membership, sizes nested expanded bodies using native resized-host anchor preview, preserves attached events/manual labels, preserves graphical group enclosure membership/margins/nesting, and routes same-owner and cross-pool MessageFlows using preserved cardinal ports or observed offset geometry classified by the installed native service. Complete record coverage, revision, precomputed intent, native persistence, fresh-worker and whole-archive gates apply. Unknown or inconsistent ports, unverified host-relative offset docking, unrepresented surfaces and unverified resized-host attachments are rejected, never silently dropped. Original retained; poll operation_get. Not full Modeler automation, live-unsaved editing or desktop visual compatibility.")]
    public CallToolResult LayoutNativeDiagram(string path, string expectedRevision, NativeDiagramLayoutRequest layout) => Guard(() => native.LayoutDiagram(path, expectedRevision, layout));
    ...
    [McpServerTool(Name = "native_elements_reparent"), Description("Experimental explicit native reparenting between processes and embedded subprocesses. Cross-diagram moves require ExpectedDiagramId and TargetDiagramId on every crossing root; migrate native values, embedded/image files and current-user tabs. Configured parameters require simulationMigration with complete source/target scenario mappings and matching units/global context/inheritance. CopyMissingDependencies explicitly permits absent resource/calendar copies; conflicts reject. Saved-result retirement requires separate explicit consent; unknown container content rejects. Nonempty action collections require migratePresentationActions=true; existing action objects, cached content and owned bytes follow their moved owners. Different-byte collisions and unrepresented records reject; no cache refresh or playback. Preserve identities, original file and connector/boundary closure. Optional Position changes only the selected node. Three workers verify persistence and archive content. Poll operation_get; not automatic layout or live editing.")]
    public CallToolResult ReparentNative(string path, string expectedRevision, NativeReparenting[] moves, NativeSimulationMigration? simulationMigration = null, bool migratePresentationActions = false) =>
        Guard(() => native.Reparent(path, expectedRevision, moves, simulationMigration, migratePresentationActions));
