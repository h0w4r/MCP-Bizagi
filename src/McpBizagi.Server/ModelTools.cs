...
public sealed class ModelTools(WorkspaceFiles files, ServerOptions options, NativeWorkflows native, Operations operations)
{
    ...
    [McpServerTool(Name = "native_mutate"), Description("Apply explicit create/update/delete/reconnect mutations to a native copy. Participant creation requires a new ProcessId; lanes/milestones use that stable process as ParentId and require complete partition geometry. Embedded expanded subprocesses require separate ExpandedSize. CallActivity accepts CallTarget.ProcessId (existing local participant process, not diagram ID); an empty ProcessId unlinks, omitted CallTarget preserves. Requires revision, fresh-worker readback and whole-archive fidelity. Handle children, incident flows and incoming calls explicitly before deletion; never overwrites the input.")]
    ...
    [McpServerTool(Name = "native_simulate"), Description("Experimental installed-engine simulation of one native diagram. Supply native diagram/scenario IDs from native_inspect. An empty scenario ID explicitly uses native defaults. Reusable subprocesses simulate their overall shape time, not linked-process internals; inspect SimulationLimitations. Level 1-4; no source edits. Poll operation_get for real results or errors.")]
