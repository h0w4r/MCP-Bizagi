...
public sealed class ModelTools(WorkspaceFiles files, ServerOptions options, NativeWorkflows native, Operations operations)
{
    ...
    public CallToolResult Capabilities()
    {
        ...
        return Result(new
        {
            ...
            nativeDiagramLayout = new { tool = "native_diagram_layout", dependency = NativeSurfaceLayoutPlanner.Dependency,
                ...
                scope = "experimental_complete_represented_diagram_pool_partition_expanded_anchor_group_midpoint_and_cross_pool_planning_with_native_persistence_and_fidelity_not_universal_or_visual_accreditation" },
    ...
    [McpServerTool(Name = "native_diagram_layout"), Description("Experimental server-calculated complete selected-diagram layout. Supply DiagramId and Direction Right or Down, not coordinates. Preserves source lane/milestone membership, sizes nested expanded bodies using native resized-host anchor preview, preserves attached events/manual labels, preserves graphical group enclosure membership/margins/nesting, and routes same-owner and cross-pool MessageFlows using preserved native midpoint ports. Complete record coverage, revision, precomputed intent, native persistence, fresh-worker and whole-archive gates apply. Unknown/offset ports, unrepresented surfaces and unverified resized-host attachments are rejected, never silently dropped. Original retained; poll operation_get. Not full Modeler automation, live-unsaved editing or desktop visual compatibility.")]
