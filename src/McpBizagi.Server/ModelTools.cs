...
public sealed class ModelTools(WorkspaceFiles files, ServerOptions options, NativeWorkflows native, Operations operations)
{
    ...
    [McpServerTool(Name = "native_diagram_layout"), Description("Experimental server-calculated complete selected-diagram layout. Supply DiagramId and Direction Right or Down, not coordinates. Preserves source lane/milestone membership, sizes nested expanded bodies, translates attached events/manual labels, preserves graphical group enclosure membership/margins/nesting, and routes same-owner and cross-pool MessageFlows using preserved native midpoint ports. Complete record coverage, revision, precomputed intent, native persistence, fresh-worker and whole-archive gates apply. Unknown/offset ports, unrepresented surfaces and boundary attachments on resized expanded hosts are rejected, never silently dropped. Original retained; poll operation_get. Not full Modeler automation, live-unsaved editing or desktop visual compatibility.")]
