...
public sealed class ModelTools(WorkspaceFiles files, ServerOptions options, NativeWorkflows native, Operations operations)
{
    ...
    public CallToolResult Capabilities()
    {
        ...
        return Result(new
        {
            ...
            nativeConversions = new { tool = "native_elements_convert", taskTypes = NativeConversionPolicy.TaskTypes, gatewayTypes = NativeConversionPolicy.GatewayTypes,
                taskToUnboundCall = new { targetType = "CallActivity", createsDiagram = false, selectsTarget = false, bindingTool = "native_mutate", bindingField = "CallTarget.ProcessId", reverseSupported = false },
                maximumBatchSize = 1000, scope = "same_category_or_task_to_unbound_call_explicit_expected_type_and_revision_no_silent_content_retirement" },
    ...
    [McpServerTool(Name = "native_elements_convert"), Description("Convert task/gateway types within the same category, or a task to an unbound CallActivity, using installed native commands. Each change requires ElementId, ExpectedType and TargetType, plus the source revision. Task-to-call creates no diagram and selects no target: bind explicitly through native_mutate CallTarget.ProcessId; reverse call-to-task is unsupported. Preserves identities, common properties, relations, styling and unknown archive content; rejects nondefault type-specific content that would be lost and attributes not applicable to the target type. Writes a new artifact and verifies it in an independent reader. Not event conversion, embedded subprocess extraction or live-session editing. Poll operation_get.")]
