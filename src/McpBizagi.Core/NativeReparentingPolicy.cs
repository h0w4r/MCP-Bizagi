...
public static partial class NativeReparentingPolicy
{
    ...
    public static NativeFidelityReport Compare(byte[] beforeBytes, byte[] afterBytes, NativeElement[] before, NativeElement[] after, NativeReparenting[] changes,
        EngineReply? sourceReply = null, EngineReply? editedReply = null, EngineReply? reopenedReply = null, NativeSimulationMigrationPlan? simulation = null, NativePresentationMigrationPlan? presentation = null)
    {
        Verify(before, after, changes);
        if (before.Any(e => after.Single(a => a.Id == e.Id).DiagramId != e.DiagramId))
            return CompareCrossDiagram(beforeBytes, afterBytes, before, after, changes, sourceReply, editedReply, reopenedReply, simulation, presentation);
