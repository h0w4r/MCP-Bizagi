...
internal static class NativeConversionAcceptance
{
        ...
        Func<string, string, Task<JsonElement>> wait, Action<string> exited, Func<string, Dictionary<string, object?>, Task<JsonElement>> error)
    {
        ...
        var created = await Op("native_model_create", new() { ["diagramNames"] = new[] { "Native conversion Ω" } });
        ...
        int index = 0;
        foreach (var family in new[] { Tasks, Gateways })
            foreach (string source in family)
                foreach (string target in family.Where(t => t != source))
                {
        ...
        conversions.Add(new(nested, "UserTask", "ManualTask"));
        ...
        string documentedTask = conversions.Single(c => c.ExpectedType == "UserTask" && c.TargetType == "ServiceTask").ElementId;
        ...
        XElement Definition(string id, string type) => new("ExtendedAttribute", new XAttribute("Id", id), new XAttribute("Type", type),
            ...
            new XElement("ElementTypes", Tasks.Select(t => new XElement("AttributeElementType", new XAttribute("Type", t)))));
        ...
        async Task VerifyRichContent(JsonElement result)
        {
            ...
            foreach (var node in beforeGraph.EnumerateArray())
            {
                ...
                foreach (string field in new[] { "ActivityLoop", "ActivityProperties", "Documentation", "Style", "ParentId", "SourceRef", "TargetRef" })
        ...
        if (converted.GetProperty("requestedChangesVerified").GetInt32() != 99) throw new InvalidDataException("Missing directed conversion pairs.");
        ...
        await error("native_elements_convert", new() { ["path"] = path, ["expectedRevision"] = originalRevision, ["changes"] = reverse });
