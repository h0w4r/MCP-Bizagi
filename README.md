...
> **Bounded delivery verified:** [0.7 package acceptance](docs/validation-package-0.7.md)
> passed six real extracted-package circuits, including live unsaved recovery,
> native creation, inlining, diagram layout and failure recovery.
>
...
| Inline a local reusable subprocess body | Experimental source after 0.6 | Native call-to-container conversion plus complete body copying; preserved shared process/other callers, explicit placement and two-stage archive/fresh-reader gates; not recursive expansion or behavioral equivalence; [contract](docs/native-inlining.md) |
| Move native elements between containers | Experimental; same-diagram in 0.6, cross-diagram in later source | Explicit process/subprocess ownership, subtree IDs, reference closure, native values/files/tab migration and optional position; explicit mapped scenario migration is available in later source; presentation-action migration requires explicit consent; auto-layout remains open; [reparenting](docs/native-reparenting.md), [scenarios](docs/native-scenario-migration.md) |
| Align/distribute selected native shapes | Experimental; 0.6 package verified | Eight installed-editor modes, typed MCP request, native callback acknowledgment, durable readback and archive fidelity; newer source preserves host-attached events and callback ports; not whole-diagram auto-layout; [contract](docs/native-layout.md) |
| Automatically place a native surface | Experimental source after 0.6 | Server-side MSAGL topology calculation, one installed-editor grouped move transaction, root/embedded surfaces, host/label packing envelopes and guarded native routes; not multi-owner/global layout or GUI compatibility; [contract](docs/native-surface-layout.md) |
| Automatically lay out a represented diagram | Experimental source after 0.6 | Server-calculated pools, lane/milestone membership, nested expanded bodies, native resized-host anchors, graphical group enclosures, manual labels and cross-pool routes; actual native offset-port classification, archive and fresh-reader gates; unsupported surfaces reject; [contract](docs/native-diagram-layout.md) |
...
| Native connector port metadata | Experimental source after 0.6 | Exact readback; explicit preserve/replace/clear on connector creation or reconnect; no automatic rerouting; [contract](docs/native-connector-ports.md) |
...
| Live unsaved Modeler sessions | Experimental; 0.7 package verified | Dedicated launch/discovery, revision-guarded Name/Documentation batches, native undo/redo, checkpoints, guarded publication and owner cleanup; MCP-tree termination preserves unsaved work; [contract](docs/live-sessions.md) |
