...
| Automatic represented-diagram layout | Experimental source after 0.6 | Complete selected-diagram MSAGL placement/routing, preserved pool/lane/milestone membership, nested expanded sizes and native resized-host anchors, graphical group membership/margins, manual labels, cardinal native ports and cross-pool messages; blank/single/rich native corpus, cancellation and recovery; not every surface, rounded curve or independent desktop compatibility; [contract](native-diagram-layout.md) |
