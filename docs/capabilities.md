...
| Task-to-unbound-call conversion | Experimental source after 0.6 | Eight task types plus a nested task, installed command, whole-archive checks, explicit attribute applicability, native I/O/event references and follow-up call binding/clearing; not extraction, reverse inlining or simulator equivalence; [contract](native-conversions.md#task-to-an-unbound-call) |
...
| Embedded-to-reusable subprocess extraction | Experimental current source | Native command at root/nested levels, preserved child identities, rich content relocation and current-user tab remapping; reverse inlining and scenario/action migration remain open; [contract](native-refactoring.md) |
| Explicit native element reparenting | Experimental; 0.6 package verified | Same-diagram process/subprocess collections, reference closure, optional root position and whole-archive preservation; cross-diagram migration and auto-layout remain open; [contract](native-reparenting.md) |
