...
| `native_elements_convert` | `path`, `expectedRevision`, `changes` | Explicit `ElementId`, `ExpectedType`, `TargetType` pairs within the native task/gateway category, or between tasks and unbound `CallActivity`; native command, target attribute scopes, fresh-reader and whole-archive fidelity; no implicit call binding/unlinking or inlining; [contract](native-conversions.md) |
