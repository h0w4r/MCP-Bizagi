...
| `native_elements_convert` | `path`, `expectedRevision`, `changes` | Explicit `ElementId`, `ExpectedType`, `TargetType` pairs within task/gateway categories, between tasks and unbound `CallActivity`, or between event kinds with mandatory same-role `ExpectedEventMode`; native commands, guarded neutral definitions, target attribute scopes, fresh-reader and whole-archive fidelity; no implicit binding/unlinking, inlining or event-role change; [contract](native-conversions.md) |
| `native_elements_reparent` | `path`, `expectedRevision`, `moves`; optional `simulationMigration`, `migratePresentationActions` | Explicit same/cross-diagram subtree ownership changes with reference closure, native content relocation and fresh-reader/whole-archive gates. Configured scenarios and existing actions require separate consent; [reparenting](native-reparenting.md), [action migration](native-action-migration.md) |
