...
| `native_elements_convert` | `path`, `expectedRevision`, `changes` | Explicit `ElementId`, `ExpectedType`, `TargetType` pairs within the native task/gateway category, or task to unbound `CallActivity`; native command, target attribute scopes, fresh-reader and whole-archive fidelity; no implicit call binding; [contract](native-conversions.md) |
